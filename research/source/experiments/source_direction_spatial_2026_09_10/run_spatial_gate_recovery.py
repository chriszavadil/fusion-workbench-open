...
def execute(row):
 folder=HERE/'runs'/row['id'];folder.mkdir(parents=True,exist_ok=True);start=time.monotonic();model,meta=make(row['layout'],row['seed'])
 s=model.settings;s.batches=A['batches'];s.particles=A['particles'];s.track=A['track_ids']
 ...
 trackfolder=folder
 if not (folder/'tracks.h5').exists():
  trackfolder=folder/'companion_tracks';trackfolder.mkdir();model.settings.batches=1;model.export_to_xml(directory=trackfolder)
  with (trackfolder/'private-execution.log').open('w') as log:
   done=subprocess.run([str(Path(sys.executable).with_name('openmc')),'-s',str(A['threads'])],cwd=trackfolder,env=env,stdin=subprocess.DEVNULL,stdout=log,stderr=subprocess.STDOUT,timeout=120)
  if done.returncode:raise RuntimeError('Track companion failed')
  model.settings.batches=A['batches']
 ...
 tracks=track_export(trackfolder);save(folder/'TRACKS.json',tracks)
 out={'case':row,'histories':s.particles*s.batches,'completed':True,'seconds':time.monotonic()-start,'tallies':tallies,'mesh':{'dimension':A['mesh_dimension'],'lower_left_cm':list(mesh.lower_left),'upper_right_cm':list(mesh.upper_right),'indices_1based':indices,'voxel_cm3':volume},'statepoint_sha256':sha(state),'tracks_hdf5_sha256':sha(trackfolder/'tracks.h5'),'tracks_json_sha256':sha(folder/'TRACKS.json'),'geometry_sha256':sha(folder/'GEOMETRY.json'),'primary_histories_recorded':tracks['source_histories_recorded'],'physical_validation':False}
 save(folder/'RESULT.json',out);return out
if __name__=='__main__':
 # Resume only missing postprocessing; existing statepoints and logs are never overwritten.
 ...
 save(HERE/'RUN_MANIFEST_RECOVERY.json',{'admission_sha256':sha(HERE/'ADMISSION.json'),'driver_sha256':sha(Path(__file__)),'material_sha256':sha(HERE/'materials_and_reference.py'),'openmc':openmc.__version__,'before_execution':True})
 status=[]
 for case in A['cases']:
  try:
