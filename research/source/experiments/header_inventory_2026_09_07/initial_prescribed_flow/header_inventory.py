...
class HeaderStudy:
    ...
    def evaluate(self,side,radius,split=True,choice='mean'):
        geom=self.geometry(side,radius);net=self.net if split else self.old
        branch=net.branch(side,net.nominal[side],choice,geom['coolant_reservation_fraction'])
        moduleflow=net.nominal[side]/self.count[side]
