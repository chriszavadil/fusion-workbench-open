...
def run(case):
 ...
 (target/'STARTED.json').write_text(json.dumps({'input_sha256':sha(inp),'driver_sha256':sha(Path(__file__)),'upstream':PIN,'adaptive_sha256':sha(ROOT/'adaptive_fatigue.py'),'windows_shim_sha256':sha(ROOT/'windows_output_fix.py'),'pulsed_mission_patch_installed':case.startswith('pulsed')},indent=2)+'\n',encoding='utf-8')
 ...
 if case.startswith('pulsed'):
  import mission_constraint
  mission_constraint.install()
 ...
 keys='ifail convergence_parameter rmajor rminor dr_cs dr_bore b_plasma_toroidal_on_axis hfact plasma_current p_fusion_total_mw p_hcd_injected_total_mw p_hcd_primary_injected_mw p_hcd_primary_extra_heat_mw p_hcd_electric_total_mw p_plant_electric_gross_mw p_plant_electric_net_mw p_plant_electric_recirc_mw p_coolant_pump_elec_total_mw p_plant_core_systems_elec_mw e_plant_net_electric_pulse_kwh f_c_plasma_bootstrap f_c_plasma_auxiliary f_c_plasma_inductive f_c_plasma_non_inductive n_cycle n_cycle_min t_plant_pulse_burn t_plant_pulse_total p_plasma_separatrix_mw p_l_h_threshold_mw p_plant_imbalance_mw p_electric_imbalance p_plant_primary_heat_mw eta_turbine eta_cd_hcd_primary eta_cd_norm_ecrh eta_hcd_primary_injector_wall_plug p_hcd_injected_max i_pulsed_plant'.split()
 feasible=not error and values.get('ifail')==1
 result={'case':case,'numerically_converged':feasible,'physical_plant_validated':False,'runtime_s':time.monotonic()-started,'error':error,'metrics':{k:values.get(k) for k in keys},'input_sha256':sha(inp),'mfile_sha256':sha(mfile) if mfile.exists() else None,'admission_sha256':sha(ROOT/'ADMISSION.json'),'screening_availability_adjusted_net_MW':(.8*3.6*values['e_plant_net_electric_pulse_kwh']/values['t_plant_pulse_total'] if case.startswith('pulsed') else .8*values['p_plant_electric_net_mw']) if feasible else None,'scope':'Architecture/allocation electrical screening only; no startup/outage energy, current-profile stability, experimental confinement, maintenance or fuel closure validated.'}
