...
CASES['adaptive30y']={'fatigue':True,'adaptive':True,'mission_years':30,'free_cs_current':True}
CASES['adaptive30y_warm_fixed']=dict(CASES['adaptive30y'])
def run(case):
    ...
    if cfg.get('mission_years'):
        text+='\n* Design intervention: allow lower CS current density with existing critical-field/current constraints *\nixc = 37\nboundl(37) = 1000000.0\nboundu(37) = 18500000.0\nj_cs_flat_top_end = 18500000.0\nlife_plant = 30.0\n'
    if case=='adaptive30y_warm_fixed':text+=(ROOT/'warm-start-overrides-corrected.txt').read_text()
    ...
    manifest['mission_sha256']=digest(ROOT/'mission_constraint.py')
    (out/'FROZEN_INPUT.json').write_text(json.dumps(manifest,indent=2)+'\n')
    ...
    try:
        ...
        if cfg['adaptive']:
            ...
            install()
        if cfg.get('mission_years'):
            import mission_constraint; mission_constraint.install()
