...
CASES['H_min_inverse']=(1.2,HERE/'H11_bridge/case_MFILE.DAT')
def run(case):
    ...
    manifest={'case':case,'cap':cap,'seed_sha256':digest(seed_path),'input_sha256':digest(inp),'seed':seed,
        ...
        'objective':'custom_min_hfact','inverse_admission_sha256':digest(HERE/'INVERSE_ADMISSION.json'),'objective_adapter_sha256':digest(HERE/'min_h_objective.py'),'radiation_convention_retained':1,'scaling_retained':34,'later_cooling_proposals_included':False}
    ...
    try:
        ...
        import min_h_objective;min_h_objective.install()
    ...
    selected={k:v.get(k) for k in KEYS+['norm_objf','hfact','t_energy_confinement','i_rad_loss','b_plasma_toroidal_on_axis','j_cs_flat_top_end','p_hcd_injected_total_mw','p_plasma_imbalance_mw','p_plant_imbalance_mw']}
