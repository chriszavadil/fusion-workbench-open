...
fetch('/api/neutronics').then(r=>{if(!r.ok)throw Error();return r.json();}).then(d=>{if(d.schema!=='fusion.neutronics-view.v1'||d.cases.length!==3||d.physical_validation!==false)throw Error();data=d;show(0);}).catch(()=>{by('neutron-title').textContent='Completed neutron study unavailable';});
