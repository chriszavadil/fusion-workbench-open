...
async function api(path,data){const res=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json','X-Workbench-Request':'local-ui-v1'},body:JSON.stringify(data)});const value=await res.json();if(!res.ok)throw new Error(value.error||'Request failed');return value;}
async function startJob(kind){if(!worker)return;try{const job=await api('/api/jobs',{kind,configuration_id:config.id});jobId=job.id;showJob(job);}catch(e){text('job-message',e.message);}}
function showJob(job){text('job-state',`${job.state.toUpperCase()} · ${job.kind==='rerun_process'?'FULL SOLVER':'RECORDED OUTPUT CHECK'}`);text('job-message',`${catalog.configurations.find(c=>c.id===job.configuration_id)?.title||job.configuration_id} — ${job.message||''}`);const running=['queued','running','cancelling'].includes(job.state);$('cancel').classList.toggle('hidden',!running);$('cancel').disabled=job.state==='cancelling';$('verify').disabled=running||!worker;$('rerun').disabled=running||!worker||!window.fullSolverConfigured;
 $('job-result').replaceChildren();if(job.result){const r=job.result;const line=document.createElement('p');line.textContent=r.passed?'Numerical check passed. Physical validation is unchanged.':'Not accepted as a converged result.';$('job-result').append(line);const avg=r.metrics?.conditional_average_net_MW??r.conditional_average_net_MW;if(avg!==undefined){const p=document.createElement('p');p.textContent=`Verified conditional average: ${fmt(avg,6)} MW`;$('job-result').append(p);}}
}
async function status(){try{const res=await fetch('/api/status',{cache:'no-store'});if(!res.ok)throw new Error();const s=await res.json();worker=s.worker==='available';window.fullSolverConfigured=s.full_solver_configured;text('connection',`Local worker available · checked ${new Date(s.checked_at*1000).toLocaleTimeString()}`);const current=s.jobs.find(j=>j.id===jobId)||s.jobs.at(-1);if(current){jobId=current.id;showJob(current);}else{$('verify').disabled=false;$('rerun').disabled=!s.full_solver_configured;}}
 catch(e){worker=false;text('connection','Viewer only · local worker offline');$('verify').disabled=true;$('rerun').disabled=true;}}
...
async function boot(){const response=await fetch('/data/catalog.json');if(!response.ok)throw new Error('Catalog unavailable');catalog=await response.json();window.workbenchCatalog=catalog;
 ...
 $('verify').onclick=()=>startJob('verify_recorded_energy');$('rerun').onclick=()=>{if(confirm('Run the approved, unchanged systems input locally? This uses one local worker, is limited to ten minutes, and does not update the accepted design.'))startJob('rerun_process');};$('cancel').onclick=async()=>{if(jobId)showJob(await api(`/api/jobs/${jobId}/cancel`,{}));};
 ...
 buildEvidence();await changeConfiguration(catalog.configurations[0].id);await status();setInterval(status,1500);requestAnimationFrame(animate);window.workbenchReady=true;
}
boot().catch(()=>{text('model-loading','Evidence catalog could not be loaded. Start the local workbench server.');text('connection','Offline');});
