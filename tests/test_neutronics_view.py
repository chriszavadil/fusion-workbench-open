...
RAW=json.loads((EXP/'KEY_RESULTS.json').read_text(encoding='utf-8'))
PACKET=json.loads((ROOT/'app/data/neutronics.json').read_text(encoding='utf-8'))
...
def test_all_six_completed_records_are_available_for_replay():
 admission=json.loads((EXP/'RUN_ADMISSION.json').read_text(encoding='utf-8'));assert len(admission['runs'])==6
 assert len({r['seed'] for r in admission['runs']})==6
 for row in admission['runs']:
  folder=EXP/'candidate_runs'/(row['layout']+'_'+str(row['seed']))
  r=json.loads((folder/'RESULT.json').read_text(encoding='utf-8'));assert r['completed'] and r['histories']==1000000
...
def test_report_in_reader_and_dashboard_scoped():
 ...
 cat=json.loads((ROOT/'app/data/catalog.json').read_text(encoding='utf-8'));track=next(t for t in cat['tracks'] if t['id']=='neutronics')
...
def test_no_public_remote_code_execution_added():
 js=(ROOT/'app/web/neutronics.js').read_text(encoding='utf-8')
 assert 'innerHTML' not in js and 'eval(' not in js and 'POST' not in js
 cpp=(ROOT/'native/Source/FusionWorkbench/FusionNeutronics.cpp').read_text(encoding='utf-8')
