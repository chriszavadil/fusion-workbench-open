...
try:
 with sync_playwright() as p:
  ...
  expect(page.locator('#progress-chart [data-event]')).to_have_count(12);expect(page.locator('#event-detail')).to_have_attribute('data-event','ec-equilibrium-interface-20260916')
  expect(page.locator('#latest-result')).to_contain_text('did not match')
  ...
  page.locator('#support-work').uncheck();expect(page.locator('#progress-chart [data-event]')).to_have_count(8);page.locator('#support-work').check()
  ...
  page.set_viewport_size({'width':1440,'height':1050});page.goto(a.base,wait_until='networkidle');expect(page.locator('#progress-overview-chart [data-event]')).to_have_count(12)
  ...
  R.update(passed=True,browser=b.version,event_markers=12,goals=6,reuse_entries=13,interactive_filters=True,shared_homepage_graph=True,narrow_layout_no_page_overflow=True,scope='Real Edge desktop and narrow viewport, not a physical-device or comprehensive accessibility audit')
  b.close()
