...
async function route(){if(['#watch','#design'].includes(location.hash)){for(let n=0;n<300&&!window.workbenchReady;n++)await new Promise(r=>setTimeout(r,100));if(location.hash==='#watch'){openView('transport');window.dispatchEvent(new Event('workbench:watch-particles'));}else openView('device');window.scrollTo({top:0,behavior:'instant'});return;}if(location.hash==='#benchmarks'){for(let n=0;n<100&&!window.workbenchReady;n++)await new Promise(r=>setTimeout(r,100));openView('benchmarks');return;}const m=location.hash.match(/^#research=(.*)$/);if(!m)return;for(let n=0;n<100&&!window.workbenchReady;n++)await new Promise(r=>setTimeout(r,100));openView('research',decodeURIComponent(m[1]));}
window.addEventListener('hashchange',route);route();

for(const a of document.querySelectorAll('a[href="#watch"],a[href="#design"]'))a.addEventListener('click',()=>{if(location.hash===a.getAttribute('href'))route();});
